...
def get_dishes(menu_id: UUID, submenu_id: UUID, dish: DishService = Depends()) -> list[dict]:
    return dish.get_all(menu_id=menu_id, submenu_id=submenu_id)


@router.get('/{dish_id}', response_model=DishOutSchema)
def get_dish(menu_id: UUID, submenu_id: UUID, dish_id: UUID, dish: DishService = Depends()) -> dict:
...
def create_dish(menu_id: UUID, submenu_id: UUID, dish_data: DishInSchema, dish: DishService = Depends()) -> dict:
...
def delete_dish(menu_id: UUID, submenu_id: UUID, dish_id: UUID, dish: DishService = Depends()) -> dict:
