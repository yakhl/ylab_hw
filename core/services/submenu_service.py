...
from ..repositories.cache_tags import all_submenus_tag
from ..repositories.error_messages import submenu_200_deleted_msg
from ..repositories.submenu_repository import SubmenuRepository
from ..schemas.submenu_schemas import SubmenuInSchema


class SubmenuService:
    def __init__(self, cache_repository: CacheRepository = Depends(), repository: SubmenuRepository = Depends()):
    ...
    def get_all(self, menu_id: UUID) -> list[dict]:
        ...
        db_submenus = self.repository.get_all(menu_id=menu_id)
    ...
    def get(self, menu_id: UUID, submenu_id: UUID) -> dict:
        ...
        db_submenu = self.repository.get(menu_id=menu_id, submenu_id=submenu_id)
        self.cache_repository.set(db_submenu['id'], db_submenu)
        return db_submenu

    def create(self, menu_id: UUID, submenu_data: SubmenuInSchema) -> dict:
        db_submenu = self.repository.create(menu_id=menu_id, submenu_data=submenu_data)
    ...
    def update(self, menu_id: UUID, submenu_id: UUID, submenu_data: SubmenuInSchema) -> dict:
        all_submenus_id = self._get_all_submenus_id(menu_id, all_submenus_tag)
        db_submenu = self.repository.update(menu_id=menu_id, submenu_id=submenu_id, submenu_data=submenu_data)
        self.cache_repository.delete(all_submenus_id)
        self.cache_repository.set(db_submenu['id'], db_submenu)
        return db_submenu

    def delete(self, menu_id: UUID, submenu_id: UUID) -> dict:
        self.repository.delete(menu_id=menu_id, submenu_id=submenu_id)
        self.cache_repository.flush()
