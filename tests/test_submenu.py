...
class TestSubmenu:
    ...
    @pytest.mark.asyncio
    async def test_read_submenus_in_empty_menu(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_read_submenus_after_create(self, client: AsyncClient):
        response = await client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus')
        assert response.status_code == 200, response.text
        data_out = response.json()
    ...
    @pytest.mark.asyncio
    async def test_update_submenu(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_read_submenus_after_update(self, client: AsyncClient):
        response = await client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus')
        assert response.status_code == 200, response.text
        data_out = response.json()
        assert isinstance(data_out, list) and len(data_out) == 1
    ...
    @pytest.mark.asyncio
    async def test_read_submenu_after_update(self, client: AsyncClient):
        response = await client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus/{SubmenuValueStorage.id}')
        assert response.status_code == 200, response.text
        data_out = response.json()
        assert data_out == {
    ...
    @pytest.mark.asyncio
    async def test_read_submenu_after_delete(self, client: AsyncClient):
        response = await client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus/{SubmenuValueStorage.id}')
