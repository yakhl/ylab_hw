from .conftest import DishValueStorage, MenuValueStorage, SubmenuValueStorage, client
...
def test_read_dishes_in_empty_submenu():
    response = client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus/{SubmenuValueStorage.id}/dishes')
...
def test_read_dishes_after_update():
    response = client.get(f'/api/v1/menus/{MenuValueStorage.id}/submenus/{SubmenuValueStorage.id}/dishes')
    assert response.status_code == 200, response.text
    data_out = response.json()
    assert isinstance(data_out, list) and len(data_out) == 1
...
def test_delete_submenu():
    response = client.delete(f'/api/v1/menus/{MenuValueStorage.id}/submenus/{SubmenuValueStorage.id}')
    assert response.status_code == 200, response.text
